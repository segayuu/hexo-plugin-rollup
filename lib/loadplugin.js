...
function createPluginInctance({ name, config }, strict) {
  if (!(is.plainObject(config) || null == config)) {
    throw new TypeError();
  }
...
const pkgObj = {
  ...
  create(plugins, strict = false) {
    ...
    if (!plugins.every(i => is.plainObject(i) && is.string(i.name) && is.plainObject(i.config))) {
