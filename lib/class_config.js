...
const { resolveArray: pluginsResolve, create: pluginsCreate, tryLoadErrors } = require("./loadplugin");
...
function setRollupPaths(plugins, paths = []){
  ...
  if (is.plainObject(plugin_paths) && is.plainObject(plugin_paths.config)) {
...
class HexoRollupConfig {
  ...
  _filterIncludeAndPaths(configInTheme) {
    let paths;
    let include;
    ...
    setRollupPaths(plugins, paths);
    setRollupIncludeJsFiles(plugins, include);

    return plugins;
  }
  getConfig(data, strict = false) {
    const { path, text: contents } = data;
    const plugins = pluginsCreate(this._filterIncludeAndPaths(this.isTheme(path)), strict);
    {
      const logger = this.hexo.log;
