...
function tryLoadModule(module_name, strict = false) {
  if (!is.string(module_name)) {
    throw new TypeError("module_name most string.");
  }
  const func = strict ? require : tryRequire;
  const result = func(module_name);
...
function loadPlugin(plugin_name, strict = false, logger = null) {
  ...
  if (!strict && plugin === false) {
    return false;
  }
  if (!is.callable(plugin)) {
    throw new Error(`plugin "${plugin_name}" module not callable!`);
  }
  const err = tryRequire.lastError;
  if (err !== loadPlugin.lastError) {
    loadPlugin.lastError = err;
    warning(err.toString);
  }
  return plugin;
}

loadPlugin.lastError = null;
