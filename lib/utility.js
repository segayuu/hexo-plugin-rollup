...
module.exports = {
  ...
  all_join(path, obj) {
    if (!isString(path)) {
      throw new TypeError('first argument "path" most string!');
    }
    if (obj == null) {
      return {};
    } else if (!isPlainObject(obj)) {
      throw new TypeError('second argument "obj" most plain object!');
    }
  ...
  base_config(config, js_dir) {
    ...
    const keys = Object.keys(config).filter(i => i !== 'include');
